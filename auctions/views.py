...
def create_listing(request):
    ...
    if request.method == "POST":
        ...
        return render(
            ...
            {"form": form, "total": total_items(request.user.id)},
        )
    ...
    if request.method == "POST":
        ...
        if "bid" in request.POST:
            ...
            if listing_instance.starting_bid < bid:
                ...
                bids = Bids_table(
                    author=user_instance, listing=listing_instance, bid=bid
                )
                bids.save()
            ...
            available = Watchlist.objects.filter(
            ...
            winner_watchlist_available = Watchlist.objects.filter(
            ...
            update_comment = Comment(
                author=user_instance, listing=listing_instance, comment=comment
            )
            update_comment.save()
            return HttpResponseRedirect(
                reverse("listing", kwargs={"listing_id": listing_instance.id})
        ...
        if (listing.closed == True) and (winning_bid.author.id == request.user.id):
            messages.success(
                request, f"Your bid of Kshs.{winning_bid.bid} was successful!"
            )
...
def watchlist(request):
    ...
    if request.method == "POST":
        ...
        return render(
            request,
            "auctions/watch.html",
            {"watch_items": watch_items, "total": total_items(request.user.id)},
        )
...
def category(request):
    if request.method == "POST":
        ...
        return render(
            ...
            {"options": options, "total": total_items(request.user.id)},
        )
